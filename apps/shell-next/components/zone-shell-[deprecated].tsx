...
export async function ZoneShellDeprecated({ eyebrow, title, description, children, links }: ZoneShellProps) {
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-5 py-10 md:px-8">
      <header className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-8 shadow-[0_20px_80px_rgba(15,23,42,0.45)]">
        ...
        <div className="mt-6 space-y-3">
          <p className="text-xs uppercase tracking-[0.3em] text-cyan-300">Main Zone routes</p>
        ...
        {links ? <div className="mt-6 space-y-3">
          <p className="text-xs uppercase tracking-[0.3em] text-cyan-300">Multi zones routes</p>
          <div className='flex flex-wrap gap-3'>
            {links}
            <a className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm" href="/shop">Shop</a>
            <a className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm" href="/shop/products">Shop products route</a>
            <a className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm" href="/dashboard">Dashboard</a>
            <a className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm" href={email ? '/me' : '/login'}>{email ? 'Profile' : 'Login'}</a>
            {email ? <a className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm" href="/logout">Logout</a> : null}
